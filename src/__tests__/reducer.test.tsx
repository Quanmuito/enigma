...
describe('Update state with invalid action type or id', () => {
    test('Set reflector to %p', () => {
        ...
        let updatedState = reducer(getAppStateByDate(0), action);
        expect(updatedState).toEqual(getAppStateByDate(0));
    });
...
const settingsByDate = [
    ['0', 'AAA', 'AAA', ''],
    ['3', 'LIQ', 'KTR', 'BJ HC PI RF UO ZQ'],
    ['25', 'MFO', 'ZBY', 'DW GO HE UF YI ZJ'],
];
describe('Update date setting', () => {
    test.each(settingsByDate)(
        'Setting date to %p',
        (value, rings, starts, plugboard) => {
            let initState = getAppStateByDate(0);
            let action = {
                type: ACTION_DATE,
                payload: {
                    value: value,
            ...
            let updatedState = reducer(initState, action);
            expect(updatedState).toEqual(getAppStateByDate(parseInt(value)));
            expect(updatedState.setting.date).toEqual(parseInt(value));
            expect(updatedState.setting.ringSettings).toEqual(rings);
            expect(updatedState.setting.startSettings).toEqual(starts);
            expect(updatedState.setting.plugboardSettings).toEqual(plugboard);
        }
    );
});
